...
# restart_travel_planning_backend.sh - Script to restart the Travel Planning Agent backend

echo "🔄 Restarting Travel Planning Agent Backend..."
...
# Check if virtual environment exists in travel planning backend
BACKEND_ROOT="$PROJECT_ROOT/applications/travel_planning_agent/backend"
...
if ! python3 -c "import fastapi" 2>/dev/null; then
    ...
    echo "Please install dependencies: pip install -r $BACKEND_ROOT/requirements.txt"
    exit 1
fi
